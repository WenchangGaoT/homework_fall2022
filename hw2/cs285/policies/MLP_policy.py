...
class MLPPolicy(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def get_action(self, obs: np.ndarray) -> np.ndarray:
        ...
        distr = self(ptu.from_numpy(observation))
...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(self, observations, actions, advantages, q_values=None):
        ...
        loss = torch.neg(torch.mean(log_probs * advantages))
        loss.backward()
        self.optimizer.step()
        ...
        if self.nn_baseline:
            ...
            targets = ptu.from_numpy(normalize(q_values, np.mean(q_values), np.std(q_values)))
